...
from llm_search_quality_evaluation.dataset_generator.models import LLMQueryResponse
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            '{"queries":["", " ", "Valid"]}',
            r"(at least 1 character|min_length|String should have at least 1)",
        ),
        ('{"queries":["Good", 123, null]}', r"(valid string|string_type)"),
    ],
)
def test_llm_service_generate_queries_with_invalid_responses__expects__error(
    invalid_response, expected_error, example_doc
):
...
def test_generate_queries_with_unicode_strings__expects__list_of_unicode_strings(
    example_doc,
):
    fake_llm = FakeListChatModel(
        responses=['{"queries":["こんにちは", "你好", "¡Hola!"]}']
    )
    service = LLMService(chat_model=FakeChatModelAdapter(fake_llm))
...
def test_generate_queries_with_leading_trailing_whitespace__expects__whitespace_stripped(
    example_doc,
):
