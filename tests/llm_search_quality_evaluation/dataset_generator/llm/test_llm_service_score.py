...
def test_generate_score_with_valid_scale__expects__integer_score(
    scale, valid_score, example_doc
):
    ...
    response = service.generate_score(
        example_doc, query, relevance_scale=scale, explanation=False
    )
...
def test_generate_score__with_invalid_json_response__expects__raises_value_error(
    example_doc,
):
    ...
    with pytest.raises(ValueError, match="Invalid LLM response:"):
        service.generate_score(
            example_doc, "query", relevance_scale="binary", explanation=True
        )


@pytest.mark.parametrize(
    "scale, valid_score, explanation",
    [
...
def test_generate_score_with_valid_explanation__expects__explanation(
    scale, valid_score, explanation, example_doc
):
...
def test_generate_score_with_invalid_llm_responses__expects__raises_value_error(
    scale, response_json, example_doc
):
...
def test_generate_score_with_invalid_relevance_scale__expects__raises_value_error(
    example_doc,
):
